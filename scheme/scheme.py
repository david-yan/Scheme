...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        values.append(scheme_eval(clause.second.first, env,
            ))
    ...
    return env.make_child_frame(list_to_pair(formals), list_to_pair(values))
...
def check_formals(formals):
    ...
    while formals is not nil:
        if formals.first in parameters or not scheme_symbolp(formals.first) or formals.first is nil:
            raise SchemeError()
