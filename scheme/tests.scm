...
(eval (define elite (+ (* (* e l) i) te)))
; expect 1337

'(h . 1)
; expect (h . 1)

(define who '(1 (am . (wood V)))
; expect who
...
(eval (cons 'cdr '('(diamond groot))))
; expect (groot)

(begin (define var (- elite 24)) (/ var 13))
; expect 101

(define lol (begin (print 'rofl) (/ var 13)))
; expect lol
...
(define (broken_printer x) (print x))
; expect broken_printer
...
(define (deep-map fn s)
  (cond 
    ((null? s) s)
    ((number? (car s)) (cons (fn (car s)) (deep-map fn (cdr s))))
...
(let ((inner 'noob)) (and 'i 'am 'an inner))
; expect noob

(or 'avoided SchemeError())
; expect avoided

(and 'L 0 'L)
; expect l

(define theory 'Saturday)
; expect theory

(cond 
  ((eq? theory 'Monday) 'early)
  ((eq? theory 'Tuesday) 'early)
  ((eq? theory 'Wednesday) 'early)
  ((eq? theory 'Thursday) 'maybe?)
  ((eq? theory 'Friday) 'nope)
  ((eq? theory 'Saturday) 'FNAF3!)
  ((eq? theory 'Sunday) '2monthslater)
  (else 'jumpscare))
; expect fnaf3!
...
(let ((the 'its) (hat 'gone)) (list the hat))
; expect (its gone)

(list the hat)
; expect (is there)

(define where (mu (is) waldo))
; expect where

(define find (lambda (look waldo) (list look (where look))))
; expect find
